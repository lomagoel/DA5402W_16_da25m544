...
def eval_best_model_version(model_name, mlflow_tracking_uri):
    ...
    latest_version_obj = client.get_latest_versions(model_name)[0]
    print(f'Latest Version: {latest_version_obj.version}')

    model_description = client.get_registered_model(model_name)
    model_aliases = model_description.aliases

    if 'best' not in model_aliases:
        client.set_registered_model_alias(model_name, 'best', latest_version_obj.version)
    else:
        ...
        latest_run = client.get_run(latest_version_obj.run_id).data
