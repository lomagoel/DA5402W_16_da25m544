...
class PreProcess(beam.DoFn):
    ...
    def setup(self):
        if self.is_train:
            ...
            self.augment = T.Compose([T.Resize(size=(224, 224))])
...
with beam.Pipeline() as pipeline:
    ...
    _ = (
        raw_images
        | "Filter Train Slice"
        >> beam.Filter(get_stratified_split_filter(target_partition=0))
        | "Reshuffle Train" >> beam.Reshuffle()
        | "Augment Train" >> beam.ParDo(PreProcess(is_train=True))
        | "Direct Disk Write Train"
        >> beam.ParDo(
            DirectImageWriter(base_output_path=os.path.join(out_gcs_root, "train"))
        )
    )
    ...
    _ = (
        raw_images
        | "Filter Val Slice"
        >> beam.Filter(get_stratified_split_filter(target_partition=1))
        | "Reshuffle Val" >> beam.Reshuffle()
        | "Augment Val" >> beam.ParDo(PreProcess(is_train=False))
        | "Direct Disk Write Val"
        >> beam.ParDo(
            DirectImageWriter(base_output_path=os.path.join(out_gcs_root, "val"))
        )
    )
    ...
    _ = (
        raw_images
        | "Filter Test Slice"
        >> beam.Filter(get_stratified_split_filter(target_partition=2))
        | "Reshuffle Test" >> beam.Reshuffle()
        | "Augment Test" >> beam.ParDo(PreProcess(is_train=False))
        | "Direct Disk Write Test"
        >> beam.ParDo(
            DirectImageWriter(base_output_path=os.path.join(out_gcs_root, "test"))
        )
    )
...
for split in splits:
    ...
    print(
        f"Split: {split:<6} | Total Images: {len(images):<5} | Unique Classes: {len(categories)}"
    )
